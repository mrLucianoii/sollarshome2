...
var SolidCalculate =  {
    ...
    custom: {
        ...
        vrBase: function ( sQft ){
            ...
            return this.baseValue + ( this.baseSQFT * sQft );
        ...
        vrCustom: function( sQft ){
           ...
           return this.customValue + ( this.customSQFT * sQft );
    ...
    singleHome:{
        ...
        branchHtml: 
            ...
            +'<li>'
            +'<input type="checkbox" id="inBranch11" class="regular-checkbox big-checkbox" /><label for="inBranch11"></label> <p>25 feet: $12000</p>'
            +'</li>'
            +'<li>'
            +'<input type="checkbox" id="inBranch12" class="regular-checkbox big-checkbox" /><label for="inBranch12"></label> <p>50 feet: $16000</p>'
            +'</li>'
            +'<li>'
            +'<input type="checkbox" id="inBranch13" class="regular-checkbox big-checkbox" /><label for="inBranch13"></label> <p>100 feet: $29000</p>'                            
            +'</li>'
            +'</ul>',
        bigBranchHtml: 
            '<ul id="input5" class="check-SD" data-inputs="singleHome">'
            ...
            +'<h4>Add a header from the admin portal </h4>'
            +'<li>'
            +'<input type="checkbox" id="crBranch11" class="regular-checkbox big-checkbox" /><label for="crBranch11"></label> <p>25 feet: $12000<a href="#" class="title-popover" data-title="25 Feet Cost: " data-body="And here\'s some amazing content. It\'s very engaging. Right?"><span class="glyphicon glyphicon-info-sign"></span></a></p>'
            +'</li>'
            +'<li>'
            +'<input type="checkbox" id="crBranch12" class="regular-checkbox big-checkbox" /><label for="crBranch12"></label> <p>50 feet: $16000<a href="#" class="title-popover" data-title="50 Feet Cost: " data-body="And here\'s some amazing content. It\'s very engaging. Right?"><span class="glyphicon glyphicon-info-sign"></span></a></p>'
            +'</li>'
            +'<li>'
            +'<input type="checkbox" id="crBranch13" class="regular-checkbox big-checkbox" /><label for="crBranch13"></label> <p>100 feet: $29000<a href="#" class="title-popover" data-title="100 Feet Cost: " data-body="And here\'s some amazing content. It\'s very engaging. Right?"><span class="glyphicon glyphicon-info-sign"></span></a></p>'                            
            +'</li>'
            +'</ul>',
        getValue: function(inputString, currentObj ){
    ...
    painting:{
        none: 0,
        baseSQFT: 4,
        paintadd: function () { return this.baseSQFT * SolidCalculate.sqFt },
        getValue: function( inputString, currentObj ){
...
//           console.log( $('ul.edit-list li a')[4].innerHTML );
        ...
        if (answer.dataString == 'sBase'){
            switch (answer.currentModel) { 
                case 'single':
                    ...
                    this.conditionOpt(this.selectedBase);
    ...
    conditionOpt: function ( selected ) {
        
        //.tree branch
        document.getElementById('sqftFLAT').innerHTML = selected * SolidCalculate.sCondtn.flat;
        document.getElementById('sqftSLOP').innerHTML = selected * SolidCalculate.sCondtn.gently
        document.getElementById('sqftHILL').innerHTML = selected * SolidCalculate.sCondtn.hillside;
        
        //.core branch
        document.getElementById('sqftFLATcore').innerHTML = selected * SolidCalculate.sCondtn.flat;
        document.getElementById('sqftSLOPcore').innerHTML = selected * SolidCalculate.sCondtn.gently
        document.getElementById('sqftHILLcore').innerHTML = selected * SolidCalculate.sCondtn.hillside;
...
var configValues = {};
/*
$.getJSON( '../../solid-admin/json/configValues/values.json', function(result) {
    
    console.log(result);
    
    configValues = result ;
    
    SolidCalculate.single = Number(configValues['modelPrices']['single']);
    SolidCalculate.guest = Number(configValues['modelPrices']['guest']);
    SolidCalculate.rental = Number(configValues['modelPrices']['rental']);
    SolidCalculate.garage = Number(configValues['modelPrices']['garage']);
    SolidCalculate.office = Number(configValues['modelPrices']['office']);
    SolidCalculate.poolHouse = Number(configValues['modelPrices']['poolHouse']);

    var solidKeys = Object.keys(SolidCalculate),
        valueKeys = Object.keys(configValues);
    
    for (var i=0; i < valueKeys.length; i++){
        
        if (valueKeys[i] != 'modelPrices'){
         
            for (var y=0; y < solidKeys.length; y++){
                
                if ( valueKeys[i] == solidKeys[y] ){
                    
                    var solidProperty = Object.keys( SolidCalculate[ solidKeys[y] ] ),
                        valueProperty = Object.keys( configValues[ solidKeys[y] ] );
                    
                        for ( var x=0; x < valueProperty.length; x++  ){
                            
                            for ( var z = 0; z < solidProperty.length; z++ ){
                                
                                if ( valueProperty[x] == solidProperty[z]  ) {
                                                                        
                                    SolidCalculate[ valueKeys[i] ][ valueProperty[z] ] = Number(configValues[ valueKeys[i] ][ valueProperty[z] ]);
